...
use crate::topology::{
    topods_edge::TopoDsEdge, topods_face::TopoDsFace, topods_shell::TopoDsShell,
    topods_solid::TopoDsSolid, topods_vertex::TopoDsVertex, topods_wire::TopoDsWire,
};
            ...
            if let Some(edge_ref) = edge.get() {
                ...
                if i > 0 {
                    if let Some(prev_edge_ref) = edges[i - 1].get() {
                        ...
                        self.connect_variable_radius_segments(
                            &mut pipe_shell,
                            prev_edge_ref,
                            edge_ref,
                            profile,
