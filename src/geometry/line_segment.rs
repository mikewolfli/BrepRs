...
impl LineSegment {
    ...
    pub fn contains(&self, point: &Point, tolerance: StandardReal) -> bool {
        ...
        if dot_product > line_vec.magnitude_squared() + tolerance {
            return false;
        }
    ...
    pub fn offset(&self, distance: StandardReal) -> (LineSegment, LineSegment) {
        ...
        let offset_start1 = self.start + offset_vec;
        let offset_end1 = self.end + offset_vec;
        
        let offset_start2 = self.start - offset_vec;
        let offset_end2 = self.end - offset_vec;
