...
impl MeshGenerator {
        ...
        match shape_type {
            crate::topology::ShapeType::Face => {
                ...
                if let Some(face) = shape.as_face() {
                    let face_handle = crate::foundation::handle::Handle::new(std::sync::Arc::new(face.clone()));
    ...
    fn bowyer_watson_algorithm(&self, mesh: &mut mesh_data::Mesh2D, vertices: &[crate::geometry::Point], mesh_vertices: &[usize]) {
        ...
        for (i, vertex) in vertices.iter().enumerate() {
            ...
            for (j, triangle) in triangles.iter().enumerate() {
                if self.point_in_circumcircle(*vertex, vertices[triangle.0], vertices[triangle.1], vertices[triangle.2]) {
            ...
            triangles.retain(|_, j| !bad_triangles.contains(&j));
        ...
        triangles.retain(|(a, b, c)| {
            !super_vertices.contains(a) && !super_vertices.contains(b) && !super_vertices.contains(c)
        });
    ...
    fn point_in_circumcircle(&self, p: crate::geometry::Point, a: crate::geometry::Point, b: crate::geometry::Point, c: crate::geometry::Point) -> bool {
        let dx = a.x - p.x;
        let dy = a.y - p.y;
        let ex = b.x - p.x;
        let ey = b.y - p.y;
    ...
    fn bowyer_watson_3d(&self, mesh: &mut crate::mesh::mesh_data::Mesh3D, vertices: &[crate::geometry::Point], mesh_vertices: &[usize], max_edge_length: f64) {
        ...
        for (i, vertex) in vertices.iter().enumerate() {
            ...
            for (j, tetra) in tetrahedrons.iter().enumerate() {
                if self.point_in_circumsphere_3d(*vertex, vertices[tetra.0], vertices[tetra.1], vertices[tetra.2], vertices[tetra.3]) {
            ...
            tetrahedrons.retain(|_, j| !bad_tetrahedrons.contains(&j));
        ...
        let super_vertices = [vertices.len(), vertices.len() + 1, vertices.len() + 2, vertices.len() + 3];
        tetrahedrons.retain(|(a, b, c, d)| {
            !super_vertices.contains(a) && !super_vertices.contains(b) && 
            !super_vertices.contains(c) && !super_vertices.contains(d)
        });
        
        // Add tetrahedrons to mesh
        for (a, b, c, d) in tetrahedrons {
            if a < mesh_vertices.len() && b < mesh_vertices.len() && 
               c < mesh_vertices.len() && d < mesh_vertices.len() {
                // Check edge lengths
                if self.check_edge_lengths(vertices, a, b, c, d, max_edge_length) {
                    mesh.add_tetrahedron(mesh_vertices[a], mesh_vertices[b], mesh_vertices[c], mesh_vertices[d]);
                }
            }
        }
    }
    ...
    fn create_super_tetrahedron(&self, vertices: &[crate::geometry::Point]) -> (usize, usize, usize, usize) {
        // Find the bounding box of all vertices
        let mut min_x = f64::INFINITY;
        let mut max_x = f64::NEG_INFINITY;
        let mut min_y = f64::INFINITY;
        ...
        // Return indices (these will be added to the vertices list)
        (vertices.len(), vertices.len() + 1, vertices.len() + 2, vertices.len() + 3)
    }
    
    /// Check if a point is inside the circumsphere of a tetrahedron
    fn point_in_circumsphere_3d(&self, p: crate::geometry::Point, a: crate::geometry::Point, b: crate::geometry::Point, c: crate::geometry::Point, d: crate::geometry::Point) -> bool {
        let mat = [
            a.x - p.x, a.y - p.y, a.z - p.z, (a.x*a.x + a.y*a.y + a.z*a.z) - (p.x*p.x + p.y*p.y + p.z*p.z),
            b.x - p.x, b.y - p.y, b.z - p.z, (b.x*b.x + b.y*b.y + b.z*b.z) - (p.x*p.x + p.y*p.y + p.z*p.z),
            c.x - p.x, c.y - p.y, c.z - p.z, (c.x*c.x + c.y*c.y + c.z*c.z) - (p.x*p.x + p.y*p.y + p.z*p.z),
            d.x - p.x, d.y - p.y, d.z - p.z, (d.x*d.x + d.y*d.y + d.z*d.z) - (p.x*p.x + p.y*p.y + p.z*p.z)
        ];
        ...
        let det = mat[0] * (mat[5] * (mat[10] * mat[15] - mat[11] * mat[14]) - mat[6] * (mat[9] * mat[15] - mat[11] * mat[13]) + mat[7] * (mat[9] * mat[14] - mat[10] * mat[13]))
                - mat[1] * (mat[4] * (mat[10] * mat[15] - mat[11] * mat[14]) - mat[6] * (mat[8] * mat[15] - mat[11] * mat[12]) + mat[7] * (mat[8] * mat[14] - mat[10] * mat[12]))
                + mat[2] * (mat[4] * (mat[9] * mat[15] - mat[11] * mat[13]) - mat[5] * (mat[8] * mat[15] - mat[11] * mat[12]) + mat[7] * (mat[8] * mat[13] - mat[9] * mat[12]))
                - mat[3] * (mat[4] * (mat[9] * mat[14] - mat[10] * mat[13]) - mat[5] * (mat[8] * mat[14] - mat[10] * mat[12]) + mat[6] * (mat[8] * mat[13] - mat[9] * mat[12]));
        
        det < 0.0
    }
    
    /// Check if a face exists in a list of faces
    fn face_exists_in_list(&self, face: (usize, usize, usize), faces: &[(usize, usize, usize)]) -> bool {
        for f in faces {
    ...
    fn check_edge_lengths(&self, vertices: &[crate::geometry::Point], a: usize, b: usize, c: usize, d: usize, max_edge_length: f64) -> bool {
    ...
    fn improve_element_quality(&self, mesh: &mut mesh_data::Mesh2D) {
        ...
        for i in 0..mesh.faces().len() {
            ...
            let edges = [(face1.a(), face1.b()), (face1.b(), face1.c()), (face1.c(), face1.a())];
                ...
                for j in i+1..mesh.faces().len() {
                    ...
                    if (face2.a() == v0 || face2.b() == v0 || face2.c() == v0) && 
                       (face2.a() == v1 || face2.b() == v1 || face2.c() == v1) {
                        ...
                        let vertices: Vec<crate::geometry::Point> = mesh.vertices().iter().map(|v| v.point).collect();
                        let quality_before = self.calculate_triangle_quality(&vertices, face1.clone()) + 
    ...
    fn calculate_triangle_quality(&self, vertices: &[crate::geometry::Point], face: mesh_data::MeshFace) -> f64 {
    ...
    fn calculate_triangle_area(&self, vertices: &[crate::geometry::Point], face: mesh_data::MeshFace) -> f64 {
        if face.vertices.len() != 3 {
            return 0.0;
        }
        let v0 = vertices[face.vertices[0]];
