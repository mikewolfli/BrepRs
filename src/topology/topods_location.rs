use crate::geometry::{Point, Transform, Direction, Axis, Vector};
...
#[derive(Debug, Clone)]
pub struct TopoDS_Location {
    translation: Point,
    transformation: Transform,
}

impl TopoDS_Location {
    ...
    pub fn inverse(&self) -> TopoDS_Location {
    ...
    pub fn multiply(&self, other: &TopoDS_Location) -> TopoDS_Location {
    ...
    pub fn distance(&self, other: &TopoDS_Location) -> f64 {
    ...
    pub fn is_equal(&self, other: &TopoDS_Location, tolerance: f64) -> bool {
        self.distance(other) < tolerance
    }
}

impl Default for TopoDS_Location {
    fn default() -> Self {
        Self::new()
    }
}

impl PartialEq for TopoDS_Location {
    fn eq(&self, other: &Self) -> bool {
        self.translation == other.translation && self.transformation == other.transformation
    }
}

impl std::ops::Mul for TopoDS_Location {
    type Output = TopoDS_Location;
...
mod tests {
    ...
    fn test_location_creation() {
        let location = TopoDS_Location::new();
    ...
    fn test_location_with_translation() {
        ...
        let location = TopoDS_Location::with_translation(translation);
    ...
    fn test_location_with_transformation() {
        ...
        let location = TopoDS_Location::with_transformation(transform.clone());
    ...
    fn test_location_is_identity() {
        ...
        let mut location2 = TopoDS_Location::new();
    ...
    fn test_location_inverse() {
        let mut location = TopoDS_Location::new();
        location.set_translation(Point::new(1.0, 2.0, 3.0));
        
        let inverse = location.inverse();
        
        // For pure translation, inverse should be (-1, -2, -3)
        assert!((inverse.translation().x - (-1.0)).abs() < 0.001, "inverse x: {}", inverse.translation().x);
        assert!((inverse.translation().y - (-2.0)).abs() < 0.001, "inverse y: {}", inverse.translation().y);
        assert!((inverse.translation().z - (-3.0)).abs() < 0.001, "inverse z: {}", inverse.translation().z);
        
        let combined = location.multiply(&inverse);
        
        // Combined translation should be identity (0, 0, 0)
        assert!((combined.translation().x).abs() < 0.001, "combined x: {}", combined.translation().x);
        assert!((combined.translation().y).abs() < 0.001, "combined y: {}", combined.translation().y);
        assert!((combined.translation().z).abs() < 0.001, "combined z: {}", combined.translation().z);
    ...
    fn test_location_multiply() {
        let mut loc1 = TopoDS_Location::new();
        loc1.set_translation(Point::new(1.0, 0.0, 0.0));
        
        let mut loc2 = TopoDS_Location::new();
    ...
    fn test_location_distance() {
        let loc1 = TopoDS_Location::with_translation(Point::new(0.0, 0.0, 0.0));
        let loc2 = TopoDS_Location::with_translation(Point::new(3.0, 4.0, 0.0));
    ...
    fn test_location_equality() {
        let loc1 = TopoDS_Location::with_translation(Point::new(1.0, 2.0, 3.0));
        let loc2 = TopoDS_Location::with_translation(Point::new(1.0, 2.0, 3.0));
