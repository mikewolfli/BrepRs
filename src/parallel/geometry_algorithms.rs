...
impl ParallelConvexHull {
    ...
    fn merge_hulls(left: &[Point], right: &[Point]) -> Vec<Point> {
        let merged: Vec<Point> = left.iter().chain(right.iter()).cloned().collect();
...
impl ParallelDelaunay {
    ...
    ) -> ParallelResult<Vec<(usize, usize, usize)>> {
        let start = Instant::now();
    ...
    fn point_in_circumcircle(p: Point, a: Point, b: Point, c: Point) -> bool {
        ...
        let _center = Point::new(ux, uy, 0.0);
    ...
    pub fn k_nearest_neighbors(
        ...
        let neighbors: Vec<Vec<(usize, f64)>> = if points.len() >= config.min_parallel_size {
            points
                .par_iter()
                .enumerate()
                .map(|(i, p)| Self::find_k_nearest(i, p, points, k))
                .collect()
        } else {
            points
                .iter()
...
impl ParallelPointCloud {
    ...
    pub fn compute_bounding_boxes(
        ...
        chunk_size: usize,
        config: &ParallelConfig,
    ) -> ParallelResult<Vec<crate::geometry::BoundingBox>> {
        let start = Instant::now();

        let chunks: Vec<&[Point]> = points.chunks(chunk_size).collect();

        let boxes: Vec<crate::geometry::BoundingBox> = if chunks.len() >= config.min_parallel_size {
            chunks
                .par_iter()
                .map(|chunk| Self::compute_bbox(chunk))
                .collect()
        } else {
            chunks
                .iter()
    ...
    fn compute_bbox(points: &[Point]) -> crate::geometry::BoundingBox {
        if points.is_empty() {
            return crate::geometry::BoundingBox::new(
...
mod tests {
    ...
    fn test_parallel_delaunay() {
        let points: Vec<Point> = (0..50)
            .map(|i| Point::new(i as f64 * 0.1, (i as f64 * 0.1).sin(), 0.0))
            .collect();
    ...
    fn test_k_nearest_neighbors() {
        let points: Vec<Point> = (0..100)
            .map(|i| Point::new(i as f64 * 0.1, 0.0, 0.0))
            .collect();
