...
use crate::topology::top_exp_explorer::TopExpExplorer;
...
impl TopExpTools {
    ...
    pub fn vertices(shape: &TopoDS_Shape) -> Vec<TopoDS_Vertex> {
    ...
    pub fn edges(shape: &TopoDS_Shape) -> Vec<TopoDS_Edge> {
    ...
    pub fn wires(shape: &TopoDS_Shape) -> Vec<TopoDS_Wire> {
        let mut wires: Vec<TopoDS_Wire> = Vec::new();
    ...
    pub fn faces(shape: &TopoDS_Shape) -> Vec<TopoDS_Face> {
        let mut faces: Vec<TopoDS_Face> = Vec::new();
    ...
    pub fn shells(shape: &TopoDS_Shape) -> Vec<TopoDS_Shell> {
    ...
    pub fn solids(shape: &TopoDS_Shape) -> Vec<TopoDS_Solid> {
    ...
    pub fn compounds(shape: &TopoDS_Shape) -> Vec<TopoDS_Compound> {
    ...
    pub fn compsolids(shape: &TopoDS_Shape) -> Vec<TopoDS_CompSolid> {
...
impl TopToolsAnalyzer {
    ...
    pub fn is_closed(shape: &TopoDS_Shape) -> bool {
        match shape.shape_type() {
            ...
            ShapeType::Shell => {
                ...
                edges.iter().all(|edge| edge_counts.get(&edge.shape_id()) == Some(&2))
            }
            _ => false,
        }
    }
    ...
    pub fn is_manifold(shape: &TopoDS_Shape) -> bool {
        match shape.shape_type() {
            ShapeType::Edge => {
            ...
            ShapeType::Shell => {
                ...
                edges.iter().all(|edge| edge_counts.get(&edge.shape_id()).unwrap_or(&0) <= &2)
            }
            ShapeType::Solid => {
    ...
    pub fn is_connected(shape: &TopoDS_Shape) -> bool {
        match shape.shape_type() {
            ...
            ShapeType::Wire => {
                ...
                for edge in &edges {
                    let v1 = edge.vertex1();
                    let v2 = edge.vertex2();
                    adjacency.entry(v1.shape_id()).or_insert_with(Vec::new).push(v2.shape_id());
                    adjacency.entry(v2.shape_id()).or_insert_with(Vec::new).push(v1.shape_id());
                }
