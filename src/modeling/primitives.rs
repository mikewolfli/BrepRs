...
use crate::geometry::{Point, Axis, Plane, Cylinder, Sphere, Direction};
use crate::topology::{TopoDS_Solid, TopoDS_Shell, TopoDS_Face, TopoDS_Wire, TopoDS_Edge, TopoDS_Vertex};
...
pub fn make_box(width: f64, height: f64, depth: f64, center: Option<Point>) -> TopoDS_Solid {
    ...
    let p1 = Point::new(center.x - half_width, center.y - half_height, center.z - half_depth);
    let p2 = Point::new(center.x + half_width, center.y - half_height, center.z - half_depth);
    let p3 = Point::new(center.x + half_width, center.y + half_height, center.z - half_depth);
    let p4 = Point::new(center.x - half_width, center.y + half_height, center.z - half_depth);
    let p5 = Point::new(center.x - half_width, center.y - half_height, center.z + half_depth);
    let p6 = Point::new(center.x + half_width, center.y - half_height, center.z + half_depth);
    let p7 = Point::new(center.x + half_width, center.y + half_height, center.z + half_depth);
    let p8 = Point::new(center.x - half_width, center.y + half_height, center.z + half_depth);
    
    // Create vertices
    let v1 = Handle::new(Arc::new(TopoDS_Vertex::new(p1)));
    let v2 = Handle::new(Arc::new(TopoDS_Vertex::new(p2)));
    let v3 = Handle::new(Arc::new(TopoDS_Vertex::new(p3)));
    let v4 = Handle::new(Arc::new(TopoDS_Vertex::new(p4)));
    let v5 = Handle::new(Arc::new(TopoDS_Vertex::new(p5)));
    let v6 = Handle::new(Arc::new(TopoDS_Vertex::new(p6)));
    let v7 = Handle::new(Arc::new(TopoDS_Vertex::new(p7)));
    let v8 = Handle::new(Arc::new(TopoDS_Vertex::new(p8)));
    
    // Create edges
    let e1 = Handle::new(Arc::new(TopoDS_Edge::new(v1.clone(), v2.clone())));
    let e2 = Handle::new(Arc::new(TopoDS_Edge::new(v2.clone(), v3.clone())));
    let e3 = Handle::new(Arc::new(TopoDS_Edge::new(v3.clone(), v4.clone())));
    let e4 = Handle::new(Arc::new(TopoDS_Edge::new(v4.clone(), v1.clone())));
    let e5 = Handle::new(Arc::new(TopoDS_Edge::new(v5.clone(), v6.clone())));
    let e6 = Handle::new(Arc::new(TopoDS_Edge::new(v6.clone(), v7.clone())));
    let e7 = Handle::new(Arc::new(TopoDS_Edge::new(v7.clone(), v8.clone())));
    let e8 = Handle::new(Arc::new(TopoDS_Edge::new(v8.clone(), v5.clone())));
    let e9 = Handle::new(Arc::new(TopoDS_Edge::new(v1.clone(), v5.clone())));
    let e10 = Handle::new(Arc::new(TopoDS_Edge::new(v2.clone(), v6.clone())));
    let e11 = Handle::new(Arc::new(TopoDS_Edge::new(v3.clone(), v7.clone())));
    let e12 = Handle::new(Arc::new(TopoDS_Edge::new(v4.clone(), v8.clone())));
    
    // Create wires (faces)
    let mut w1 = TopoDS_Wire::new();
    w1.add_edge(e1.clone());
    w1.add_edge(e2.clone());
    w1.add_edge(e3.clone());
    w1.add_edge(e4.clone());
    
    let mut w2 = TopoDS_Wire::new();
    w2.add_edge(e5.clone());
    w2.add_edge(e6.clone());
    w2.add_edge(e7.clone());
    w2.add_edge(e8.clone());
    
    let mut w3 = TopoDS_Wire::new();
    w3.add_edge(e1.clone());
    w3.add_edge(e9.clone());
    w3.add_edge(e5.clone());
    w3.add_edge(e10.clone());
    
    let mut w4 = TopoDS_Wire::new();
    w4.add_edge(e2.clone());
    w4.add_edge(e10.clone());
    w4.add_edge(e6.clone());
    w4.add_edge(e11.clone());
    
    let mut w5 = TopoDS_Wire::new();
    w5.add_edge(e3.clone());
    w5.add_edge(e11.clone());
    w5.add_edge(e7.clone());
    w5.add_edge(e12.clone());
    
    let mut w6 = TopoDS_Wire::new();
    ...
    // Create faces
    let f1 = TopoDS_Face::with_outer_wire(w1);
    let f2 = TopoDS_Face::with_outer_wire(w2);
    let f3 = TopoDS_Face::with_outer_wire(w3);
    let f4 = TopoDS_Face::with_outer_wire(w4);
    let f5 = TopoDS_Face::with_outer_wire(w5);
    let f6 = TopoDS_Face::with_outer_wire(w6);
...
pub fn make_sphere(radius: f64, center: Option<Point>) -> TopoDS_Solid {
    let center = center.unwrap_or(Point::origin());
    ...
    let face = TopoDS_Face::with_surface(Handle::new(Arc::new(sphere)));
...
pub fn make_cylinder(radius: f64, height: f64, center: Option<Point>, axis: Option<Axis>) -> TopoDS_Solid {
    let center = center.unwrap_or(Point::origin());
    ...
    let side_face = TopoDS_Face::with_surface(Handle::new(Arc::new(cylinder)));
    
    // 2. Bottom face (circle)
    let bottom_center = center;
    let bottom_plane = Plane::new(bottom_center, *axis.direction(), Direction::x_axis());
    let bottom_face = TopoDS_Face::with_surface(Handle::new(Arc::new(bottom_plane)));
    ...
    let top_face = TopoDS_Face::with_surface(Handle::new(Arc::new(top_plane)));
...
pub fn make_cone(radius1: f64, _radius2: f64, height: f64, center: Option<Point>, axis: Option<Axis>) -> TopoDS_Solid {
    let center = center.unwrap_or(Point::origin());
    let axis = axis.unwrap_or(Axis::z_axis());
...
pub fn make_torus(_major_radius: f64, minor_radius: f64, center: Option<Point>) -> TopoDS_Solid {
    let center = center.unwrap_or(Point::origin());
...
pub fn make_prism(_wire: &TopoDS_Wire, _vector: &crate::geometry::Vector) -> TopoDS_Solid {
...
pub fn make_revolution(_wire: &TopoDS_Wire, axis: &Axis, _angle: f64) -> TopoDS_Solid {
