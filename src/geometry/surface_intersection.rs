use crate::foundation::types::Standard_Real;
use crate::geometry::{Point, Vector, Direction, Plane, Sphere, Cylinder};
...
impl SurfaceIntersection3D {
    ...
    pub fn plane_plane(plane1: &Plane, plane2: &Plane, tolerance: Standard_Real) -> SurfaceIntersection {
        ...
        if cross_mag < tolerance {
            ...
            let d1 = -(n1.x * plane1.location().x + n1.y * plane1.location().y + n1.z * plane1.location().z);
            let d2 = -(n2.x * plane2.location().x + n2.y * plane2.location().y + n2.z * plane2.location().z);
    ...
    pub fn plane_sphere(plane: &Plane, sphere: &Sphere, tolerance: Standard_Real) -> SurfaceIntersection {
        ...
        if dist > radius + tolerance {
            ...
            let sign = if dx * normal.x + dy * normal.y + dz * normal.z > 0.0 { 1.0 } else { -1.0 };
            let point = Point::new(
            ...
            for i in 0..32 {
                let angle = 2.0 * std::f64::consts::PI * (i as Standard_Real) / 32.0;
                let cos_a = angle.cos();
                let sin_a = angle.sin();
    ...
    pub fn plane_cylinder(plane: &Plane, cylinder: &Cylinder, tolerance: Standard_Real) -> SurfaceIntersection {
        let plane_loc = plane.location();
        ...
        let dot = plane_normal.x * cyl_axis.x + plane_normal.y * cyl_axis.y + plane_normal.z * cyl_axis.z;
        let angle = dot.abs();
    ...
    pub fn sphere_sphere(sphere1: &Sphere, sphere2: &Sphere, tolerance: Standard_Real) -> SurfaceIntersection {
...
impl SurfaceOperations3D {
    ...
    pub fn point_plane_distance(point: &Point, plane: &Plane) -> Standard_Real {
        let plane_loc = plane.location();
        let normal = plane.normal();
    ...
    pub fn point_plane_signed_distance(point: &Point, plane: &Plane) -> Standard_Real {
        let plane_loc = plane.location();
        let normal = plane.normal();
    ...
    pub fn point_sphere_distance(point: &Point, sphere: &Sphere) -> Standard_Real {
    ...
    pub fn is_point_on_plane(point: &Point, plane: &Plane, tolerance: Standard_Real) -> bool {
        Self::point_plane_distance(point, plane) <= tolerance
    }
    
    /// Check if a point is on a sphere within tolerance
    pub fn is_point_on_sphere(point: &Point, sphere: &Sphere, tolerance: Standard_Real) -> bool {
