...
impl NurbsCurve3D {
    ...
    pub fn evaluate(&self, parameter: StandardReal) -> Point {
        ...
        for i in 0..=self.degree {
            let degree_usize = self.degree as usize;
            let index = span - degree_usize + i as usize;
            let weight = self.weights[index] * basis[i as usize];
            let pole = self.poles[index];
            weighted_sum =
                weighted_sum + Vector::new(pole.x * weight, pole.y * weight, pole.z * weight);
            weight_sum += weight;
        }

        if weight_sum > STANDARD_REAL_EPSILON {
            let inv_weight = 1.0 / weight_sum;
            weighted_sum = Point::new(
                weighted_sum.x * inv_weight,
                weighted_sum.y * inv_weight,
                weighted_sum.z * inv_weight,
            );
        }
    ...
    pub fn evaluate_derivative(&self, parameter: StandardReal, order: i32) -> Vector {
        ...
        for i in 0..=self.degree {
            let degree_usize = self.degree as usize;
            let index = span - degree_usize + i as usize;
            let weight = self.weights[index] * basis[order as usize][i as usize];
            let point = self.poles[index];
            weighted_sum =
                weighted_sum + Vector::new(point.x * weight, point.y * weight, point.z * weight);
            weight_sum += weight;
        }
    ...
    fn basis_functions(&self, span: usize, parameter: StandardReal) -> Vec<StandardReal> {
        ...
        for j in 1..=self.degree {
            ...
            for r in 0..j {
                ...
                let j_minus_r = (j - r) as usize;
                let temp = basis[r_usize] / (right[r_plus_1] + left[j_minus_r]);
                basis[r_usize] = saved + right[r_plus_1] * temp;
                saved = left[j_minus_r] * temp;
            }
    ...
    fn basis_functions_derivatives(
        &self,
        span: usize,
        parameter: StandardReal,
        order: i32,
        basis: &mut Vec<Vec<StandardReal>>,
    ) {
        ...
        for j in 1..=self.degree {
            ...
            for r in 0..j {
                ...
                let r_plus_1 = (r + 1) as usize;
                let j_minus_r = (j - r) as usize;
                let j_minus_1 = (j - 1) as usize;
                ndu[j_usize][r_usize] = right[r_plus_1] + left[j_minus_r];
                let temp = ndu[r_usize][j_minus_1] / ndu[j_usize][r_usize];

                ndu[r_usize][j_usize] = saved + right[r_plus_1] * temp;
                saved = left[j_minus_r] * temp;
            }
        ...
        for r in 0..=self.degree {
            ...
            for k in 1..=order {
                ...
                if r >= k {
                    let pk_plus_1 = (pk + 1) as usize;
                    let rk_usize = rk as usize;
                    a[s2][0] = a[s1][0] / ndu[pk_plus_1][rk_usize];
                    let rk_usize = rk as usize;
                    let pk_usize = pk as usize;
                    d = a[s2][0] * ndu[rk_usize][pk_usize];
                }
                ...
                for j in j1..=j2 {
                    ...
                    let pk_plus_1 = (pk + 1) as usize;
                    let pk_usize = pk as usize;
                    let index = (j - rk) as usize;
                    a[s2][index] = (a[s1][index] - a[s1][index - 1]) / ndu[pk_plus_1][j_usize];
                    d += a[s2][index] * ndu[j_usize][pk_usize];
                }
                ...
                if r <= pk {
                    ...
                    let r_usize = r as usize;
                    let pk_usize = pk as usize;
                    let index = (self.degree - r) as usize;
                    a[s2][index] = -a[s1][index - 1] / ndu[pk_plus_1][r_usize];
                    d += a[s2][index] * ndu[r_usize][pk_usize];
                }

                let k_usize = k as usize;
                let r_usize = r as usize;
                basis[k_usize][r_usize] = d;
...
mod tests {
    ...
    fn test_nurbs_curve3d_length() {
        let degree = 1;
        let poles = vec![Point::new(0.0, 0.0, 0.0), Point::new(3.0, 0.0, 0.0)];
