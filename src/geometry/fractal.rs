...
impl FractalGenerator {
    ...
    pub fn generate_mesh(&self) -> Mesh3D {
        match &self.params.fractal_type {
            ...
            FractalType::SierpinskiTetrahedron { level } => {
                self.generate_sierpinski_tetrahedron(*level)
            }
            ...
            FractalType::Mandelbulb { iterations, power } => {
                self.generate_mandelbulb(*iterations, *power)
            }
            FractalType::JuliaSet {
                iterations,
                constant,
            } => self.generate_julia_set(*iterations, *constant),
            FractalType::LSystemTree {
                iterations,
                angle,
                length,
            } => self.generate_l_system_tree(*iterations, *angle, *length),
            FractalType::Terrain {
                size,
                height,
                roughness,
            } => self.generate_terrain(*size, *height, *roughness),
        }
    }
    ...
    fn generate_sierpinski_tetrahedron(&self, level: usize) -> Mesh3D {
        ...
        let v0 = Point::new(
            center.x,
            center.y + scale,
            center.z + scale / std::f64::consts::SQRT_3,
        );
        let v1 = Point::new(
            center.x - scale,
            center.y - scale,
    ...
    fn generate_koch_snowflake(&self, level: usize) -> Mesh3D {
        ...
        let edges = vec![(v0, v1), (v1, v2), (v2, v0), (v0, v3), (v1, v3), (v2, v3)];

        let subdivided_edges: Vec<(Point, Point)> = edges
            .iter()
    ...
    fn mandelbulb_gradient(&self, point: &Point, iterations: usize, power: f64) -> Vector {
        ...
        let fx = self.mandelbulb_evaluate(
            &Point::new(point.x + epsilon, point.y, point.z),
            iterations,
            power,
        );
        let fy = self.mandelbulb_evaluate(
            &Point::new(point.x, point.y + epsilon, point.z),
            iterations,
            power,
        );
        let fz = self.mandelbulb_evaluate(
            &Point::new(point.x, point.y, point.z + epsilon),
            iterations,
            power,
        );

        Vector::new((fx - f) / epsilon, (fy - f) / epsilon, (fz - f) / epsilon).normalize()
    }
    ...
    fn julia_gradient(
        ...
        let fx = self.julia_evaluate(
            &Point::new(point.x + epsilon, point.y, point.z),
            iterations,
            constant,
        );
        let fy = self.julia_evaluate(
            &Point::new(point.x, point.y + epsilon, point.z),
            iterations,
            constant,
        );
        let fz = self.julia_evaluate(
            &Point::new(point.x, point.y, point.z + epsilon),
            iterations,
            constant,
        );
    ...
    fn generate_terrain(&self, size: usize, height: f64, roughness: f64) -> Mesh3D {
        ...
        // Initialize corners
        heightmap[0][0] = (rng.gen::<f64>() - 0.5) * height;
        heightmap[0][size] = (rng.gen::<f64>() - 0.5) * height;
        heightmap[size][0] = (rng.gen::<f64>() - 0.5) * height;
        heightmap[size][size] = (rng.gen::<f64>() - 0.5) * height;
        ...
        while step > 1 {
            ...
            for y in (0..size).step_by(step) {
                for x in (0..size).step_by(step) {
                    ...
                    heightmap[y + half][x + half] =
                        avg + (rng.gen::<f64>() - 0.5) * scale_factor * height;
                }
            }
            ...
            for y in (0..=size).step_by(half) {
                for x in ((y + half) % step..=size).step_by(step) {
                    ...
                    heightmap[y][x] =
                        sum / count + (rng.gen::<f64>() - 0.5) * scale_factor * height;
                }
            }
    ...
    fn create_cylinder(&self, mesh: &mut Mesh3D, start: &Point, end: &Point, radius: f64) {
        ...
        for i in 0..segments {
            ...
            ring_start.push(Point::new(
                start.x + offset.x,
                start.y + offset.y,
                start.z + offset.z,
            ));
            ring_end.push(Point::new(
                end.x + offset.x,
                end.y + offset.y,
                end.z + offset.z,
            ));
...
mod tests {
    ...
    fn test_julia_set() {
        let params = FractalParams {
            fractal_type: FractalType::JuliaSet {
                iterations: 10,
                constant: (-0.8, 0.156, 0.0),
            },
            center: Point::origin(),
            scale: 1.0,
        };
    ...
    fn test_l_system_tree() {
        let params = FractalParams {
            fractal_type: FractalType::LSystemTree {
                iterations: 2,
                angle: 0.5,
                length: 0.2,
            },
    ...
    fn test_terrain() {
        let params = FractalParams {
            fractal_type: FractalType::Terrain {
                size: 16,
                height: 0.5,
                roughness: 0.5,
            },
